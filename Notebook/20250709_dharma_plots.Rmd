...
plot_model(total_model, type = "pred", terms = c("da_factor", "age.in.months"), show.intercept = TRUE, show.p = TRUE) 
...
for(j in 1:nrow(journals)) { 
  ...
  model <- two_term_glmnb(nsd_yes_metadata, journals[[j,1]])
  # assign(journal_abrev[[j,1]], model)
  
 plot_model <- plot_model(model, type = "pred", terms = c("da_factor", "age.in.months")) +     ggtitle(paste0("Predicted counts of 'is.referenced.by.count' \nPlotted for journal ", journals[[j,1]]))
 plot_model
