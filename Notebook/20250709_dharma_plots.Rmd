...
ratio_plot <- 
  journal_data %>% 
    ...
    group_by(age.in.months) %>% 
    count(da_factor, .drop = FALSE) %>%  
    mutate(da_yes_no_ratio_by_age = `n`[da_factor == "Yes"]/`n`[da_factor == "No"]) %>% 
    ggplot(aes(x = age.in.months, y = da_yes_no_ratio_by_age)) + 
      geom_point() + 
     ggtitle(paste0("Ratio of da = Yes to da = No \nPlotted for journal ", journals[[j,1]]))
...
print(actual_citations)
  ...
  if(j == 3) { #genomea = 5
  plot_model <- plot_model(model, type = "pred", terms = c("da_factor", "age.in.months[12,60]")) +     ggtitle(paste0("Predicted counts of 'is.referenced.by.count' \nPlotted for journal ", journals[[j,1]]))
  }
  else if (j == 9) { ##Mra - 7 
