...
all_journals <- read_csv("~/Documents/Schloss/Colovas_Data_Accessibility/Data/negative_binomial/all_data_glmnb_models.csv") %>%  
  mutate(coefficients = str_replace_all(coefficients, "da_factorYes", "da_Yes"), 
         coefficients = str_replace_all(coefficients, "log\\(age.in.months\\)", "log(time)"), 
         coefficients = str_remove_all(coefficients, "container.title"))
...
knitr::kable(all_journals, digits = 3, col.names = gsub("_", " ", names(all_journals)), cap = "log(time) = log(age.in.months), shortened for ease of reading") 
