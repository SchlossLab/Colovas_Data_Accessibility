...
all_journals %>%
  pivot_longer(cols = c(full, at_one_year, at_five_years, five_years, at_ten_years, ten_years), names_to = "model", values_to = "model_value") %>%
  pivot_longer(cols = contains("pvalue"), names_to = "model_pvalues", values_to = "pvalue") %>% view()
  ggplot(aes(y = pretty_name, x = model_value)) + 
  geom_line(na.rm = TRUE) + 
  geom_point(aes(color = model, shape = (pvalue <= 0.05)), alpha = 0.75, na.rm = TRUE)
```
