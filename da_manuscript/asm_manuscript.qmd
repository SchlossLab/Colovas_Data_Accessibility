...
To fully answer our research questions, we created a dataset with papers curated from reference databases Crossref, NCBI, Scopus, and the Web of Science (@rcrossref, @sayers_ncbi_2020 , @rscopus, @wos). These papers span all twelve ASM journals of interest from January 1st, 2000 to December 31st, 2024. The ASM Journals of interest were *Applied and Envrionmental Microbiology; Antimicrobial Agents and Chemotherapy; Infection and Immunity; Journal of Clinical Biology; Journal of Virology; Journal of Bacteriology; Journal of Microbiology and Biology Education; Microbiology Resource Announcments* (formerly known as *Genome Announcements*); *mSystems; mSphere; mBio; and Microbiology Spectrum.* The data was updated as of February 10th, 2025 with all citation counts frozen at that date. See results for number of papers from each journal of interest. With a dataset this large, we needed machine learning tools to classify papers as new seqeuncing papers and to determine if those new seqeuncing papers contained available data.

### Cleaning data for modeling

To perform the computational steps required for these experiments, we used the python tool Snakemake (@mölder_snakemake), and the University of Michigan's high performance computing cluster (see acknowledgements). Using our selected papers, we downloaded the entirety of each paper's source HTML and used the command line tool wget. This allowed us to use the same source HTML multiple times for updated analyses without needing to re-query the ASM webservers numerous times. Next, we filtered the HTML using R packages rvest (@rvest), textstem(@textstem), and xml2 (@xml2) to get the desired portions of the paper from the HTML including the abstract, the body of paper, all tables and figures with captions, as well as the side panels for all papers, but especially those that contained the data availability statements in papers published after a 2023 change in webpage format. Towards the goal of the fewest meaningful number of words, we used the 'Snowball' (@stopwords) dictionary of 'stop words' with R packages tm(text manipulation)(@tm_article, @tm_manual) and textstem (@textstem) to remove non-meaningful words such as articles 'a', 'an', and 'the', and other unnecessary words in each paper. We also converted all text to lowercase, concurrent with the removal of digits and non-alphabetic characters such as whitespace. To have the fewest number of unique words, we lemmatized (sorted words by grouping inflected or variant forms of the same word) words to trace them back to their root words and eliminated any possible issues with word tense. After this, we created and counted our 'tokens', phrases of up to 1-3 consecutive words from the text of the paper using R package tokeinziers (@tokenizers). We removed the 'space' character with an underscore in multi-word tokens for ease of processing, and created a count table for the number of tokens in each paper.
...
To train our random forest machine learning models, we first created an appropriate training data set. For our initial training set, we chose a set of papers from across each journal and the time period of interest, adding special emphasis to include papers that were part of our desired set of interest (i.e. contained published data) to ensure that our two models could adequately characterize each paper as a new sequencing paper and if it published raw sequencing data or not. These papers were chosen with dplyr's slice_n(). After creating our initial dataset, it was necessary for us to to identify the status of both variables by hand and determined if each paper contained "new sequencing data", and if each one had "data available". This was completed by opening each paper in an internet browser window, and searching for a "data availability" or similar statement. See Table 1XXX for specific cases and how each of these cases were identified for the purpose of this study.

| Scenario | New Sequencing Data Status |
|----------------------------------|--------------------|
| Paper is about generating new sequencing data | Yes |
| Paper uses sequencing as a confirmation of experimental technique (i.e. confirmation of plasmid insertion) | No |
