...
Data availability is a newly emerging portion of the scientific process
in the digital age. With the latest and greatest methodologies available
across fields, increasing amounts of data are being generated each day,
especially in the biological sciences. The data generated by this study
alone has over 150,000 records.

Availability of study data and metadata (data about data) is a necessary
resource for appropriate use and re-use of data and protocols. We
believe that data ``available on request'' is time consuming at best,
and renders data completely available at worst. At best, a reader in
search of data may email the corresponding author and maybe get the data
back after days, weeks, or months, or if a response is received, the
author is no longer at the publishing institution , or has no access to
the dataset of interest, or more forms are needed to complete the
request for data (@langille\_available\_2018). At worst, no email reply
is received and the reader never obtains the data requested.
...
x sequence'' (MIxS) ((2)). These standards are checklists useable by
...
((3)). Neither the JDDCP nor the FAIR principles are enforcable by any
...
With the advent of next generation sequencing, microbiology reserach has
...
until 2018, when it was rebranded to \emph{Microbiology Resource
...
database. New genome annoucements have a high percentage of NSD papers,
...
Spectrum} and its rebrand. From 2013 until the fall of 2021,
...
\emph{Microbiology Spectrum} (b. 2013, rebrand 2021), \emph{mSphere} (b.
2016), \emph{mSystems} (b. 2016), and \emph{Genome Announcements}
...
ASM journals between 2000 and 2024. After webscraping there were YYYY
...
YYYY\% of papers with NSD, had DA. See table YYYY for percentages of NSD
and DA for each journal. The journal with the highest rate of NSD was
XXXXX at XXXX\%, and the lowest was XXXX at XXXX\%. The journal with the
highest rate of DA was XXXXX at XXXX\%, and the lowest was XXXX at
XXXX\%. This was expected as \emph{Microbiology Resource Annoucements}
publishes mainly new genomic sequence data and makes the data available.
On average, papers in the dataset had XXXX citations/article. This
number varies by journal, see table YYYY for data by journal. The
